...
\begin{problemparts}
  ...
  \item \texttt{delete\_first()} If list is empty
    ...
    \texttt{L.head} to \texttt{L.head.next}. If it was \texttt{None}
    (i.e. there was only one node), set \texttt{L.tail} to
    \texttt{None} too.

  \item \texttt{delete\_last()} If list is empty (i.e. \texttt{L.head}
    is \texttt{None}), return. Else, set \texttt{L.tail} to
    \texttt{L.head.prev}. If it was \texttt{None} (i.e. there was only
    one node), set \texttt{L.head} to \texttt{None} too.
  \end{itemize}
